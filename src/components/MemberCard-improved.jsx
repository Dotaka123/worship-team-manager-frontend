...
const MemberCard = ({ member, onClick, onEdit, onDelete }) => {
  ...
  return (
    <div 
      onClick={onClick}
      className={`${genderStyle.bg} border ${genderStyle.border} rounded-lg p-4 sm:p-5 hover:border-opacity-50 transition-all cursor-pointer hover:scale-[1.02] hover:shadow-lg`}
    >
      ...
      <div className="flex items-start justify-between mb-3 sm:mb-4 gap-2">
          ...
          <div className="relative shrink-0">
            {member.photo ? (
              <img
                ...
                className={`w-10 h-10 sm:w-12 sm:h-12 rounded-md object-cover ring-2 ${genderStyle.ring}`}
              />
            ) : (
              <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-md ${genderStyle.avatarBg} flex items-center justify-center text-xs sm:text-sm font-medium ${genderStyle.text} border border-neutral-700`}>
                {initials || <User className="w-4 h-4 sm:w-5 sm:h-5" />}
              </div>
            )}
            {/* Badge genre */}
            <span className={`absolute -bottom-0.5 -right-0.5 sm:-bottom-1 sm:-right-1 text-xs bg-neutral-900 rounded-full w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center border border-neutral-700 ${genderStyle.text}`}>
              {genderIcon}
            </span>
          </div>
        ...
        <div className="flex gap-0.5 sm:gap-1 shrink-0">
          <button
            onClick={onEdit}
            className="p-1 sm:p-1.5 text-neutral-500 hover:text-indigo-400 hover:bg-indigo-950/20 rounded-md transition-colors"
            title="Modifier"
          >
          ...
          <button
            onClick={onDelete}
            className="p-1 sm:p-1.5 text-neutral-500 hover:text-red-400 hover:bg-red-950/20 rounded-md transition-colors"
            title="Supprimer"
          >
      ...
      {/* Informations - Responsive spacing */}
      <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
        ...
        {member.email && (
          <div 
            ...
            className="flex items-center gap-2 sm:gap-3 text-neutral-400 hover:text-neutral-200 transition-colors group"
          >
            <Mail className="w-4 h-4 text-neutral-500 shrink-0 group-hover:text-neutral-400" />
            <a href={`mailto:${member.email}`} className="truncate text-xs sm:text-sm">
              {member.email}
            </a>
          </div>
        )}
        ...
        {member.phone && (
          ...
          >
            ...
            <a href={`tel:${member.phone}`} className="text-xs sm:text-sm">
              {member.phone}
            </a>
          </div>
        )}
      ...
      {(member.dateEntree || member.notesAccompagnement) && (
        <div className="mt-3 sm:mt-4 pt-3 border-t border-neutral-800 space-y-2">
          {member.dateEntree && (
            <div className="flex items-center gap-2 text-xs text-neutral-500">
              <Calendar className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0" />
              <span className="truncate">
                Depuis le {new Date(member.dateEntree).toLocaleDateString('fr-FR')}
              </span>
            </div>
          )}
