...
const AttendanceTable = ({ members = [], attendance = [], onMark }) => {
  ...
  const handleStatusClick = (memberId, newStatus) => {
    ...
    // Si on clique sur le même statut, on annule
    if (currentStatus === newStatus) {
      handleClearStatus(memberId);
      return;
    }
    
    // Présent ou Absent = enregistrement direct sans délai
  ...
  const handleClearStatus = async (memberId) => {
    if (!confirm('Voulez-vous annuler le statut de présence pour ce membre ?')) {
      return;
    }
    ...
    try {
      // On envoie null comme statut pour supprimer l'enregistrement
      await onMark({
        memberId,
        status: null,
        reason: null,
  ...
  const handleMark = async (memberId, status) => {
    ...
    try {
      ...
      // Réinitialiser les inputs après succès
      setActiveInputs(prev => {
        const newState = { ...prev };
        delete newState[memberId];
        return newState;
              ...
              <tr key={member._id} className="border-b border-gray-800 hover:bg-gray-900/50">
                ...
                <td className="py-4 px-4">
                  <div className="flex justify-center gap-2 flex-wrap items-center">
                    {/* Bouton Annuler (visible si un statut existe) */}
                    {status && (
                      <button
                        onClick={() => handleClearStatus(member._id)}
                        disabled={isLoading}
                        className="px-2 py-2 rounded-lg text-sm font-medium transition bg-gray-800 text-red-400 hover:bg-red-600/20 hover:text-red-300 disabled:opacity-50 disabled:cursor-not-allowed"
                ...
                <td className="py-4 px-4">
                  ...
                  // Champ Heure - En retard ou Présent
                  : activeInput === 'time' ? (
                    <div className="flex gap-2">
                  ...
                  : status === 'excused' ? (
                    <div className="flex justify-between items-center gap-2">
                      <span 
                        className="text-sm text-violet-300 break-words flex-1" 
                        title={reason}
                      >
