...
const AttendanceTable = ({ members = [], attendance = [], onMark, onClear }) => {
  ...
  // activeInputs stocke { memberId: { type: 'time'|'reason', pendingStatus: 'present'|'en_retard'|'excused' } }
  ...
  const handleSelectChange = async (memberId, newStatus) => {
    ...
    // En retard → demander l'heure (pendingStatus = 'en_retard')
    if (newStatus === 'en_retard') {
      setActiveInputs(prev => ({ ...prev, [memberId]: { type: 'time', pendingStatus: 'en_retard' } }));
      return;
    }

    // Présent → demander l'heure optionnelle (pendingStatus = 'present')
    if (newStatus === 'present') {
      setActiveInputs(prev => ({ ...prev, [memberId]: { type: 'time', pendingStatus: 'present' } }));
      return;
    }

    // Excusé → demander le motif
    if (newStatus === 'excused') {
      setActiveInputs(prev => ({ ...prev, [memberId]: { type: 'reason', pendingStatus: 'excused' } }));
      return;
    }

    // Absent → enregistrement direct
  ...
  // Confirme avec l'heure actuelle si aucune heure saisie (pour "présent")
  const handleConfirmTime = async (memberId) => {
    const input = activeInputs[memberId];
    const pendingStatus = input?.pendingStatus || 'en_retard';
    await handleMark(memberId, pendingStatus);
  };
      ...
      <table className="w-full">
        ...
        <tbody>
          {members.map((member) => {
            ...
            const activeInput = activeInputs[member._id];          // { type, pendingStatus } ou undefined
            const pendingStatus = activeInput?.pendingStatus;
            ...
            return (
              <tr key={member._id} className="border-b border-gray-800 hover:bg-gray-900/50">
                ...
                <td className="py-4 px-4">
                  ...
                  /* Champ Heure — Présent ou En retard */
                  : activeInput?.type === 'time' ? (
                    <div className="flex gap-2">
                      ...
                      {/* Le bouton confirme avec le BON pendingStatus (present ou en_retard) */}
                      <button
                        onClick={() => handleConfirmTime(member._id)}
                        disabled={isLoading || (pendingStatus === 'en_retard' && !arrivalTime)}
                        className={`px-3 py-2 rounded text-white transition disabled:opacity-50 disabled:bg-gray-600 ${
                    ...
                    <div className="flex justify-between items-center gap-2">
                      ...
                      <button
                        onClick={() => {
                          ...
                          setActiveInputs(prev => ({ ...prev, [member._id]: { type: 'reason', pendingStatus: 'excused' } }));
                        }}
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <button
                        onClick={() => {
                          ...
                          setActiveInputs(prev => ({ ...prev, [member._id]: { type: 'time', pendingStatus: status } }));
                        }}
