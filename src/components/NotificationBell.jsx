...
const NotificationBell = () => {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      // 4. Vérifier les cotisations non payées (mois en cours)
      try {
        const currentMonth = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}`;
        const { data: cotisations } = await api.get(`/cotisations/month/${currentMonth}`);
          ...
          unpaid.forEach(c => {
            if (c.membre) {
              notifs.push({
                ...
                member: c.membre,
                amount: c.montant,
                message: `💰 ${c.membre.firstName} ${c.membre.lastName} n'a pas payé (${c.montant.toLocaleString()} Ar)`,
                timestamp: new Date()
              });
            }
          });
