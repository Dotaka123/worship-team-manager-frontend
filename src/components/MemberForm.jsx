...
const MemberForm = ({ member, onSubmit, onClose }) => {
  ...
  // Scroll automatique vers le formulaire au montage (Android)
  useEffect(() => {
  ...
  return (
    /* Overlay */
    <div
      className="fixed inset-0 bg-neutral-950/80 z-50 sm:overflow-y-auto"
      onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}
    >
      {/* 
        Mobile  → bottom sheet : glisse depuis le bas, prend toute la largeur
      ...
      */}
      <div
        ...
        className={[
          /* commun */
          'bg-neutral-900 border border-neutral-800 w-full shadow-2xl',
          'flex flex-col',
          /* mobile : collé en bas, hauteur max 92vh, coins arrondis en haut */
          'fixed bottom-0 left-0 right-0 max-h-[92vh] rounded-t-2xl',
          /* desktop : redevient un modal centré */
          'sm:static sm:relative sm:rounded-xl sm:max-h-none sm:max-w-2xl',
          'sm:mx-auto sm:my-8',
        ].join(' ')}
      >
        
        {/* Barre de glissement (mobile uniquement) */}
        <div className="sm:hidden flex justify-center pt-3 pb-1">
          <div className="w-10 h-1 bg-neutral-600 rounded-full" />
        </div>

        {/* En-tête */}
        <div className="flex justify-between items-center px-6 py-4 border-b border-neutral-800 shrink-0 bg-neutral-900 rounded-t-2xl sm:rounded-t-xl">
