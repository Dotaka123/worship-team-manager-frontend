...
const Attendance = () => {
  ...
  const handleClear = async ({ memberId }) => {
    try {
      await attendanceAPI.cancel({ memberId, date: selectedDate });
      // Retirer l'enregistrement de l'état local
      setAttendance(prev =>
        prev.filter(a => String(a.member?._id || a.member) !== String(memberId))
      );
    } catch (err) {
